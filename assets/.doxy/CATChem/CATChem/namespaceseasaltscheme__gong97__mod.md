...
# Namespace seasaltscheme\_gong97\_mod



[**Namespace List**](namespaces.md) **>** [**seasaltscheme\_gong97\_mod**](namespaceseasaltscheme__gong97__mod.md)
...
| Type | Name |
| ---: | :--- |
|  subroutine, public | [**compute\_gong97**](#function-compute_gong97) (integer, intent(in) num\_layers, integer, intent(in) num\_species, type([**seasaltschemegong97config**](namespaceseasaltcommon__mod.md#none-seasaltschemegong97config)), intent(in) params, real(fp), intent(in) frocean, real(fp), intent(in) frseaice, real(fp), intent(in) sst, real(fp), intent(in) u10m, real(fp), intent(in) v10m, real(fp), dimension(num\_species), intent(in) species\_density, real(fp), dimension(num\_species), intent(in) species\_radius, real(fp), dimension(num\_species), intent(in) species\_lower\_radius, real(fp), dimension(num\_species), intent(in) species\_upper\_radius, real(fp), dimension(num\_layers, num\_species), intent(in) species\_conc, real(fp), dimension(num\_layers, num\_species), intent(inout) species\_tendencies, real(fp), intent(inout), optional seasalt\_mass\_emission\_total, real(fp), intent(inout), optional seasalt\_number\_emission\_total, real(fp), dimension(:), intent(inout), optional seasalt\_mass\_emission\_per\_bin, real(fp), dimension(:), intent(inout), optional seasalt\_number\_emission\_per\_bin, integer, dimension(:), intent(in), optional diagnostic\_species\_id) <br>_Pure science computation for gong97 scheme._  |
|  pure subroutine, public | [**compute\_gong97**](#function-compute_gong97) (integer, intent(in) num\_layers, integer, intent(in) num\_species, type([**seasaltschemegong97config**](namespaceseasaltcommon__mod.md#none-seasaltschemegong97config)), intent(in) params, real(fp), dimension(num\_layers), intent(in) delp, real(fp), intent(in) frocean, real(fp), intent(in) frseaice, real(fp), intent(in) sst, real(fp), intent(in) u10m, real(fp), intent(in) v10m, real(fp), dimension(num\_species), intent(in) species\_density, real(fp), dimension(num\_species), intent(in) species\_radius, real(fp), dimension(num\_species), intent(in) species\_lower\_radius, real(fp), dimension(num\_species), intent(in) species\_upper\_radius, real(fp), dimension(num\_layers, num\_species), intent(in) species\_conc, real(fp), dimension(num\_layers, num\_species), intent(inout) species\_tendencies, real(fp), intent(inout), optional seasalt\_mass\_emission\_total, real(fp), intent(inout), optional seasalt\_number\_emission\_total, real(fp), dimension(:), intent(inout), optional seasalt\_mass\_emission\_per\_bin, real(fp), dimension(:), intent(inout), optional seasalt\_number\_emission\_per\_bin, integer, dimension(:), intent(in), optional diagnostic\_species\_id) <br>_Pure science computation for gong97 scheme._  |
...
### function compute\_gong97 

_Pure science computation for gong97 scheme._ 
```Fortran
subroutine, public seasaltscheme_gong97_mod::compute_gong97 (
...
This is a pure computational kernel implementing Gong 1997 sea salt emission scheme. NO error checking, validation, or infrastructure concerns. Host model must ensure all inputs are valid before calling.
...
_Pure science computation for gong97 scheme._ 
```Fortran
pure subroutine, public seasaltscheme_gong97_mod::compute_gong97 (
    integer, intent(in) num_layers,
    integer, intent(in) num_species,
    type( seasaltschemegong97config ), intent(in) params,
...
* `species_conc` Species concentrations [mol/mol] (num\_layers, num\_species) 
* `species_tendencies` Species tendency terms [mol/mol/s] (num\_layers, num\_species) 
* `seasalt_mass_emission_total` Sea salt mass emission flux total [kg/m2/s] 
* `seasalt_number_emission_total` Sea salt number emission flux total [kg/m2/s] 
* `seasalt_mass_emission_per_bin` Sea salt mass emission flux per bin [kg/m2/s] (num\_species) 
* `seasalt_number_emission_per_bin` Sea salt number emission flux per bin [kg/m2/s] (num\_species) 
* `diagnostic_species_id` Indices mapping diagnostic species to species array (optional, for per-species diagnostics) 
...
------------------------------
The documentation for this class was generated from the following file `src/process/seasalt/schemes/SeaSaltScheme_GONG97_Mod copy.F90`
