!> \file test_GasChem_integration.F90
!! \brief Comprehensive integration tests for GasChem process using CATChemCore
!!
...
!! Generated on: 2026-08-24T17:51:00.852944
...
program test_GasChem_integration
   ...
   ! Test parameters for realistic chemistry scenario
   ...
   write(output_unit,'(A)') '=================================='
   write(output_unit,'(A)') '=== GASCHEM INTEGRATION TESTS ==='
   write(output_unit,'(A)') '=================================='
   ...
   call builder%init()
   builder = builder%with_name('GasChemIntegrationTest')
   ...
   ! Register GasChem processes with ProcessFactory
   process_mgr_ptr => core%get_process_manager()
   call register_GasChem_process(process_mgr_ptr, rc)
   if (rc /= CC_SUCCESS) then
      write(error_unit,'(A)') 'ERROR: Failed to register GasChem processes with ProcessFactory'
      all_tests_passed = .false.
      goto 999
   end if
   write(output_unit,'(A)') '  ✓ GasChem processes registered with ProcessFactory'
   ...
   ! Step 3: Testing GasChem process with all schemes
   write(output_unit,'(A)') ''
   write(output_unit,'(A)') 'Step 3: Testing GasChem process with all schemes...'

   ! Add GasChem process for scheme testing
   call core%add_process('GasChem', rc)
   if (rc /= CC_SUCCESS) then
      write(error_unit,'(A)') 'ERROR: Failed to add GasChem process for scheme testing'
      all_tests_passed = .false.
      goto 999
   end if
   write(output_unit,'(A)') '  ✓ GasChem process added successfully'

   write(output_unit,'(A)') ''
   write(output_unit,'(A)') '  Testing multiple GasChem schemes...'
   ...
   if (all_tests_passed) then
      ...
      write(output_unit,'(A)') '    - GasChem process stability verified'
...
999 continue
   ...
   write(output_unit,'(A)') '=================================='
   if (all_tests_passed) then
      write(output_unit,'(A)') '=== ALL GASCHEM TESTS PASSED! ==='
      write(output_unit,'(A)') '=== Integration test successful ==='
   else
      write(output_unit,'(A)') '=== SOME GASCHEM TESTS FAILED ==='
...
contains
   ...
   !> Set up realistic meteorological conditions for GasChem testing
   subroutine setup_met(core_arg, rc_arg)
      ...
      ! Set realistic conditions for GasChem processes
      do j = 1, ny
   ...
   !> Test a specific GasChem scheme with comprehensive validation
   subroutine test_scheme(core_arg, scheme_name, rc_arg)
      ...
      type(StateManagerType), pointer :: state_mgr
      type(ProcessGasChemInterface), pointer :: GasChem_interface
      ...
      ! Get GasChem process interface
      GasChem_interface => null()
      select type(process => process_mgr%processes(1)%item)
      type is (ProcessGasChemInterface)
         GasChem_interface => process
      end select

      if (.not. associated(GasChem_interface)) then
         rc_arg = CC_FAILURE
         return
      end if

      ! Step 1: Set the timestep for process calculations
      call GasChem_interface%set_timestep(dt)

      ! Step 2: Set the scheme
      call GasChem_interface%set_scheme(scheme_name)
      ...
      case ('no_phot')
         call GasChem_interface%process_config%load_no_phot_config(config_mgr, error_mgr)
      case default
      ...
      call reset_diagnostics_for_scheme(GasChem_interface, state_mgr, scheme_name, rc_arg)
   ...
   subroutine reset_diagnostics_for_scheme(GasChem_interface, container, scheme_name, rc_arg)
      type(ProcessGasChemInterface), intent(inout) :: GasChem_interface
      ...
      ! Get current scheme
      current_scheme = GasChem_interface%get_scheme()

      ! Remove existing process registration (this clears all diagnostic fields)
      call diag_mgr%remove_process('GasChem', rc_arg)
      if (rc_arg /= CC_SUCCESS) then
         call error_mgr%report_error(ERROR_UNSUPPORTED_OPERATION, &
                                   'Failed to remove existing diagnostics for GasChem process', rc_arg)
      ...
      call GasChem_interface%register_diagnostics(container, rc_arg)
      if (rc_arg /= CC_SUCCESS) then
         call error_mgr%report_error(ERROR_UNSUPPORTED_OPERATION, &
   ...
   subroutine validate_results(core_arg, rc_arg)
      ...
      type(DiagnosticManagerType), pointer :: diag_mgr
      type(DiagnosticRegistryType), pointer :: GasChem_registry
      ...
      write(output_unit,'(A)') '  Validating GasChem emission results...'
      ...
      ! Get the GasChem process diagnostic registry
      call diag_mgr%get_process_registry('GasChem', GasChem_registry, local_rc)
      if (local_rc /= CC_SUCCESS .or. .not. associated(GasChem_registry)) then
         write(error_unit,'(A)') '  ERROR: Could not get GasChem process registry'
         rc_arg = CC_FAILURE
         return
      end if

      ! Get the number of registered diagnostic fields
      num_fields = GasChem_registry%get_field_count()
      write(output_unit,'(A,I0,A)') '    Found ', num_fields, ' registered diagnostic fields for GasChem process'

      if (num_fields == 0) then
         write(error_unit,'(A)') '  ERROR: No diagnostic fields registered for GasChem process'
         rc_arg = CC_FAILURE
         return
      end if
      ...
      ! Get all field names
      call GasChem_registry%list_fields(field_names, num_fields)
      ...
      do i = 1, num_fields
         ...
         call diag_mgr%get_field_value('GasChem', field_name, &
      ...
      if (.not. validation_passed) then
         write(error_unit,'(A)') '  VALIDATION FAILED: Some GasChem diagnostics failed validation'
         rc_arg = CC_FAILURE
      else
         write(output_unit,'(A)') '  ✓ All GasChem diagnostic validations passed'
...
end program test_GasChem_integration