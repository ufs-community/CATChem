!> \file test_dust_integration.F90
!! \brief Comprehensive integration tests for dust process using CATChemCore
!!
...
program test_dust_integration
   ...
   write(output_unit,'(A)') '=================================='
   write(output_unit,'(A)') '=== DUST INTEGRATION TESTS ==='
   write(output_unit,'(A)') '=================================='
   ...
   call builder%init()
   builder = builder%with_name('DustIntegrationTest')
   ...
   ! Register dust processes with ProcessFactory
   process_mgr_ptr => core%get_process_manager()
   call register_dust_process(process_mgr_ptr, rc)
   if (rc /= CC_SUCCESS) then
      write(error_unit,'(A)') 'ERROR: Failed to register dust processes with ProcessFactory'
      all_tests_passed = .false.
      goto 999
   end if
   write(output_unit,'(A)') '  ✓ Dust processes registered with ProcessFactory'
   ...
   ! Step 3: Testing dust process with all schemes
   write(output_unit,'(A)') ''
   write(output_unit,'(A)') 'Step 3: Testing dust process with all schemes...'

   ! Add dust process for scheme testing
   call core%add_process('dust', rc)
   if (rc /= CC_SUCCESS) then
      write(error_unit,'(A)') 'ERROR: Failed to add dust process for scheme testing'
      all_tests_passed = .false.
      goto 999
   end if
   write(output_unit,'(A)') '  ✓ Dust process added successfully'

   write(output_unit,'(A)') ''
   write(output_unit,'(A)') '  Testing multiple dust schemes...'
   ...
   if (all_tests_passed) then
      ...
      write(output_unit,'(A)') '    - Dust process stability verified'
...
999 continue
   ...
   write(output_unit,'(A)') '=================================='
   if (all_tests_passed) then
      write(output_unit,'(A)') '=== ALL DUST TESTS PASSED! ==='
      write(output_unit,'(A)') '=== Integration test successful ==='
   else
      write(output_unit,'(A)') '=== SOME DUST TESTS FAILED ==='
...
contains
   ...
   !> Set up realistic meteorological conditions for dust testing
   subroutine setup_met(core_arg, rc_arg)
      ...
      do j = 1, ny
         ...
         do i = 1, nx
            ...
            met_state%FRSNO(i,j) = 0.0_fp                      ! No snow
            ...
            met_state%LAI(i,j) = 0.0_fp                        ! Leaf area index (ocean)
   ...
   !> Test a specific dust scheme with comprehensive validation
   subroutine test_scheme(core_arg, scheme_name, rc_arg)
      ...
      type(StateManagerType), pointer :: state_mgr
      type(ProcessDustInterface), pointer :: dust_interface
      ...
      if (.not. associated(dust_interface)) then
         rc_arg = CC_FAILURE
         return
      end if

      ! Step 1: Set the timestep for process calculations
      call dust_interface%set_timestep(dt)

      ! Step 2: Set the scheme
      call dust_interface%set_scheme(scheme_name)
      ...
      call reset_diagnostics_for_scheme(dust_interface, state_mgr, scheme_name, rc_arg)
   ...
   subroutine reset_diagnostics_for_scheme(dust_interface, container, scheme_name, rc_arg)
      type(ProcessDustInterface), intent(inout) :: dust_interface
      ...
      ! Get current scheme
      current_scheme = dust_interface%get_scheme()
   ...
   subroutine validate_results(core_arg, rc_arg)
      ...
      type(DiagnosticManagerType), pointer :: diag_mgr
      type(DiagnosticRegistryType), pointer :: dust_registry
      ...
      write(output_unit,'(A)') '  Validating dust emission results...'
      ...
      ! Get the dust process diagnostic registry
      call diag_mgr%get_process_registry('dust', dust_registry, local_rc)
      if (local_rc /= CC_SUCCESS .or. .not. associated(dust_registry)) then
         write(error_unit,'(A)') '  ERROR: Could not get dust process registry'
         rc_arg = CC_FAILURE
         return
      end if

      ! Get the number of registered diagnostic fields
      num_fields = dust_registry%get_field_count()
      write(output_unit,'(A,I0,A)') '    Found ', num_fields, ' registered diagnostic fields for dust process'

      if (num_fields == 0) then
         write(error_unit,'(A)') '  ERROR: No diagnostic fields registered for dust process'
         rc_arg = CC_FAILURE
         return
      end if
      ...
      ! Get all field names
      call dust_registry%list_fields(field_names, num_fields)
      ...
      if (.not. validation_passed) then
         write(error_unit,'(A)') '  VALIDATION FAILED: Some dust diagnostics failed validation'
         rc_arg = CC_FAILURE
      else
         write(output_unit,'(A)') '  ✓ All dust diagnostic validations passed'
...
end program test_dust_integration
