!> \file test_settling_integration.F90
!! \brief Comprehensive integration tests for settling process using CATChemCore
!!
...
program test_settling_integration
   ...
   write(output_unit,'(A)') '=================================='
   write(output_unit,'(A)') '=== SETTLING INTEGRATION TESTS ==='
   write(output_unit,'(A)') '=================================='
   ...
   call builder%init()
   builder = builder%with_name('SettlingIntegrationTest')
   ...
   ! Register settling processes with ProcessFactory
   process_mgr_ptr => core%get_process_manager()
   call register_settling_process(process_mgr_ptr, rc)
   if (rc /= CC_SUCCESS) then
      write(error_unit,'(A)') 'ERROR: Failed to register settling processes with ProcessFactory'
      all_tests_passed = .false.
      goto 999
   end if
   write(output_unit,'(A)') '  ✓ Settling processes registered with ProcessFactory'
   ...
   write(output_unit,'(A)') ''
   write(output_unit,'(A)') 'Step 3: Testing settling process with all schemes...'

   ! Add settling process for scheme testing
   call core%add_process('settling', rc)
   if (rc /= CC_SUCCESS) then
      write(error_unit,'(A)') 'ERROR: Failed to add settling process for scheme testing'
      all_tests_passed = .false.
      goto 999
   end if
   write(output_unit,'(A)') '  ✓ Settling process added successfully'

   write(output_unit,'(A)') ''
   write(output_unit,'(A)') '  Testing multiple settling schemes...'
   ...
   if (all_tests_passed) then
      ...
      write(output_unit,'(A)') '    - Settling process stability verified'
...
999 continue
   ...
   write(output_unit,'(A)') '=================================='
   if (all_tests_passed) then
      write(output_unit,'(A)') '=== ALL SETTLING TESTS PASSED! ==='
      write(output_unit,'(A)') '=== Integration test successful ==='
   else
      write(output_unit,'(A)') '=== SOME SETTLING TESTS FAILED ==='
...
contains
   ...
   subroutine setup_met(core_arg, rc_arg)
      ...
      ! Set realistic conditions for settling processes
      do j = 1, ny
   ...
   !> Test a specific settling scheme with comprehensive validation
   subroutine test_scheme(core_arg, scheme_name, rc_arg)
      ...
      type(StateManagerType), pointer :: state_mgr
      type(ProcessSettlingInterface), pointer :: settling_interface
      ...
      if (.not. associated(settling_interface)) then
         rc_arg = CC_FAILURE
         return
      end if

      ! Step 1: Set the timestep for process calculations
      call settling_interface%set_timestep(dt)

      ! Step 2: Set the scheme
      call settling_interface%set_scheme(scheme_name)
      ...
      call reset_diagnostics_for_scheme(settling_interface, state_mgr, scheme_name, rc_arg)
   ...
   subroutine reset_diagnostics_for_scheme(settling_interface, container, scheme_name, rc_arg)
      type(ProcessSettlingInterface), intent(inout) :: settling_interface
      ...
      ! Get current scheme
      current_scheme = settling_interface%get_scheme()
   ...
   subroutine validate_results(core_arg, rc_arg)
      ...
      type(DiagnosticManagerType), pointer :: diag_mgr
      type(DiagnosticRegistryType), pointer :: settling_registry
      ...
      write(output_unit,'(A)') '  Validating settling emission results...'
      ...
      ! Get the settling process diagnostic registry
      call diag_mgr%get_process_registry('settling', settling_registry, local_rc)
      if (local_rc /= CC_SUCCESS .or. .not. associated(settling_registry)) then
         write(error_unit,'(A)') '  ERROR: Could not get settling process registry'
         rc_arg = CC_FAILURE
         return
      end if

      ! Get the number of registered diagnostic fields
      num_fields = settling_registry%get_field_count()
      write(output_unit,'(A,I0,A)') '    Found ', num_fields, ' registered diagnostic fields for settling process'

      if (num_fields == 0) then
         write(error_unit,'(A)') '  ERROR: No diagnostic fields registered for settling process'
         rc_arg = CC_FAILURE
         return
      end if
      ...
      ! Get all field names
      call settling_registry%list_fields(field_names, num_fields)
      ...
      if (.not. validation_passed) then
         write(error_unit,'(A)') '  VALIDATION FAILED: Some settling diagnostics failed validation'
         rc_arg = CC_FAILURE
      else
         write(output_unit,'(A)') '  ✓ All settling diagnostic validations passed'
