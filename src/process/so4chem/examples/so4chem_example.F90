!> \file so4chem_example.F90
!! \brief Example usage of so4chem process
!!
...
program so4chem_example
   ...
   ! Configuration
   character(len=256) :: config_file = "so4chem_config.yaml"
   ...
   write(output_unit, '(A)') "=== SO4chem Process Example ==="
   write(output_unit, '(A)') "Process for computing chemical production of sulfate from SO2 oxidation"
...
end program so4chem_example
write(error_unit, '(A)') "Error creating so4chem process"
call error_handler%print_errors()
stop 1
end if
write(output_unit, '(A)') "5. SO4chem process created"
...
if (error_handler%has_error()) then
   write(error_unit, '(A)') "Error initializing so4chem process"
   call error_handler%print_errors()
   stop 1
end if
write(output_unit, '(A)') "7. SO4chem process initialized"
...
subroutine setup_chemical_species(state_manager, error_handler)
   ...
   ! Add so4chem species
   call state_manager%add_species('h2o2', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('oh', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('no3', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('dms', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('so2', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('so4', error_handler)
   if (error_handler%has_error()) return
...
subroutine setup_initial_conditions(state_manager, error_handler)
   ...
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         ...
         call state_manager%set_species_concentration('h2o2', &
   ...
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('oh', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('no3', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('dms', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('so2', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('so4', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
