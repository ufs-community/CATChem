!> \file carbchem_example.F90
!! \brief Example usage of carbchem process
!!
!! This program demonstrates how to use the carbchem process
!! in a standalone application or host model integration.
!!
!! Generated on: 2026-03-29T14:20:16.534618
!! Author: Wei Li

program carbchem_example
   ...
   ! Configuration
   character(len=256) :: config_file = "carbchem_config.yaml"
   ...
   write(output_unit, '(A)') "=== CarbChem Process Example ==="
   write(output_unit, '(A)') "Process for computing chemical production and loss of carbon species"
   write(output_unit, '(A)') "Author: Wei Li"
   write(output_unit, '(A)') ""

   ! Step 1: Initialize state manager
   call state_manager%init(n_levels, n_columns, 4, rc)
...
end program carbchem_example
write(error_unit, '(A)') "Error creating carbchem process"
call error_handler%print_errors()
stop 1
end if
write(output_unit, '(A)') "5. CarbChem process created"
...
if (error_handler%has_error()) then
   write(error_unit, '(A)') "Error initializing carbchem process"
   call error_handler%print_errors()
   stop 1
end if
write(output_unit, '(A)') "7. CarbChem process initialized"
...
subroutine setup_chemical_species(state_manager, error_handler)
   ...
   ! Add carbchem species
   call state_manager%add_species('oc1', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('oc2', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('bc1', error_handler)
   if (error_handler%has_error()) return
   call state_manager%add_species('bc2', error_handler)
   if (error_handler%has_error()) return
...
subroutine setup_initial_conditions(state_manager, error_handler)
   ...
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         ...
         call state_manager%set_species_concentration('oc1', &
   ...
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('oc2', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('bc1', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
   do i_col = 1, n_columns
      do i_lev = 1, n_levels
         initial_concentration = 1.0e-9_fp  ! Default initial concentration
         call state_manager%set_species_concentration('bc2', &
            i_col, i_lev, initial_concentration, error_handler)
         if (error_handler%has_error()) return
      end do
   end do
