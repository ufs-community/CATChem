...
!! Generation date: 2026-08-24T17:51:00.274982
...
module ProcessGasChemInterface_Mod
   ...
   use Constants, only: AIRMW, RSTARG  ! Required constants for scheme calculations
   ...
   type, extends(ColumnProcessInterface) :: ProcessGasChemInterface
      ...
      ! MICM solver and state (initialized by process creator)
...
contains
   ...
   subroutine process_init(this, container, rc)
      ...
      ! Initialize MICM solver and state for reuse across columns
      grid_mgr => container%get_grid_manager()
      if (associated(grid_mgr)) then
         call grid_mgr%get_shape(nx, ny, nz)
      else
         nz = 1
      end if

      ! Create MICM solver using mechanism specified in scheme config
      this%micm => micm_t(this%process_config%no_phot_config%mechanism, RosenbrockStandardOrder, micm_error)
      if (.not. micm_error%is_success()) then
         tmp_string = micm_error%message()
         err_msg = tmp_string%get_char_array()
         write(*,*) "--- MICM INITIALIZATION DETAILED ERROR ---"
         write(*,*) "Error Message: ", trim(err_msg)
         write(*,*) "------------------------------------------"
         call error_manager%report_error(2001, 'Failed to create MICM solver in process_init', rc)
         rc = CC_FAILURE
         return
      end if

      ! Create a state sized to the model vertical dimension (nz)
      this%micm_state => this%micm%get_state(nz, micm_error)
      if (.not. micm_error%is_success()) then
         call error_manager%report_error(2002, 'Failed to create MICM state in process_init', rc)
         rc = CC_FAILURE
         return
      end if
   ...
   subroutine run_active_scheme_column(this, column, rc)
      ...
      case default
         call CC_Error('Unknown GasChem scheme: ' // &
            trim(this%process_config%GasChem_config%scheme), rc, &
            ThisLoc='run_active_scheme_column (in module ProcessGasChemInterface_Mod.F90)')
      end select
   ...
   subroutine run_no_phot_scheme_column(this, column, rc)
      ...
      allocate(sphu(n_levels))  ! Atmospheric field - always n_levels
      ...
      sphu(1:n_levels) = met%QV(1:n_levels)  ! Atmospheric field - always n_levels
      ...
      if (this%process_config%GasChem_config%diagnostics) then
         ...
         call compute_no_phot( &
            ...
            t, &
            tstep(1)            , &
            this%process_config%GasChem_config%species_names, &
            species_conc, &
            species_tendencies, &
            local_rc, &
            this%micm, this%micm_state, &
